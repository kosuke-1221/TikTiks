...
<%
    ...
    shiftData.put(year + "-" + (month + 1) + "-10", new String[]{"10:00 - 19:00", "田中 一郎","山田 太郎"});
    shiftData.put(year + "-" + (month + 1) + "-11", new String[]{"10:00 - 19:00", "田中 一郎","鈴木 花子"});
    shiftData.put(year + "-" + (month + 1) + "-12", new String[]{"10:00 - 19:00", "山田 太郎","田中 一郎"});
    shiftData.put(year + "-" + (month + 1) + "-14", new String[]{"9:00 - 18:00", "山田 太郎", "鈴木 花子"});
    shiftData.put(year + "-" + (month + 1) + "-15", new String[]{"休み"});
    shiftData.put(year + "-" + (month + 1) + "-17", new String[]{"9:00 - 18:00", "田中 一郎", "鈴木 花子"});
            ...
            if (started && day <= daysInMonth) {
                ...
                if (shiftData.containsKey(dateKey)) {
                    ...
                    if (shiftInfo.length > 1) {
                        for (int k = 1; k < shiftInfo.length; k++) {
                            String name = shiftInfo[k];
                            String cssClass = "name-" + name.replaceAll(" ", "").toLowerCase(); // クラス名生成
                            out.print("<br><small class='" + cssClass + "'>" + name + "</small>");
                        }
                    }
                }
                out.print("</td>");
