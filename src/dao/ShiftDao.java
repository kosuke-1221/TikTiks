...
public class ShiftDao {
    ...
    public List<ShiftDetail> getAllShiftDetails() throws ClassNotFoundException {
        ...
        ResultSet rs = null;
        // nameカラムを直接使用
        String sql = "SELECT s.user_id, s.shift_date, s.start_time, s.end_time, s.note, u.name " +
        ...
        try {
            ...
            while (rs.next()) {
                ...
                sd.setUserName(rs.getString("name")); // nameカラムから直接取得
                details.add(sd);
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public List<ShiftDetail> getShiftDetailsByDate(String date) throws ClassNotFoundException {
        List<ShiftDetail> details = new ArrayList<>();
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        ...
        try {
            ...
            while (rs.next()) {
                ...
                sd.setEndTime(rs.getTime("end_time").toString());
                sd.setNote(rs.getString("note"));
                sd.setUserName(rs.getString("name"));
                details.add(sd);
            }
        } catch (SQLException e) {
            e.printStackTrace();
